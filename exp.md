...
## 2026-04-06 rd Sliding-Window Rebuild

Goal:

- revisit the `rd` branch after simplifying the training code and restoring a clean timed tuning workflow
- test whether `1024` full-coverage sliding windows outperform the old `1024` random-crop pipeline

300-second reference and tuning:

- reference baseline:
  - `EXP-RD-SLIDING-BASELINE-300-001`
...
- full-coverage sliding-window comparison:
  - `EXP-RD-SLIDING-300-002`
    - change: `data.random_crop=false`, `data.sliding_window_stride=1024`
    - result: `2.422784344175074`
    - conclusion: useful
  - `EXP-RD-SLIDING-300-003`
    - change: `data.random_crop=false`, `data.sliding_window_stride=512`
    - result: `2.3037683321482296`
    - conclusion: useful

- stride neighborhood around the new baseline:
  - `EXP-RD-SLIDING-300-004`
    - change: `data.sliding_window_stride=256`
    - result: `3.2007620330916446`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-005`
    - change: `data.sliding_window_stride=384`
    - result: `2.8975163639434687`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-006`
    - change: `data.sliding_window_stride=768`
    - result: `3.3747011853710123`
    - conclusion: worse

- learning-rate sweep on top of `stride=512`:
  - `EXP-RD-SLIDING-300-007`
    - change: `training.learning_rate=5e-4`
    - result: `3.072189261190166`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-008`
    - change: `training.learning_rate=7e-4`
    - result: `2.882074626804121`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-009`
    - change: `training.learning_rate=8e-4`
    - result: `2.762234331283692`
    - conclusion: worse

- batch-size sweep on top of `stride=512`:
  - `EXP-RD-SLIDING-300-010`
    - change: `training.batch_size=6`
    - result: `3.342595467472292`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-011`
    - change: `training.batch_size=12`
    - result: `3.4011293977525017`
    - conclusion: worse
  - `EXP-RD-SLIDING-300-012`
    - change: `training.batch_size=16`
    - result: `2.998826434264549`
    - conclusion: worse

Long-budget promotion:

- `EXP-RD-SLIDING-LONG-001`
  - change: promote the strongest `300s` sliding-window candidate to the standard `7200s` budget
  - setup:
    - `max_length=1024`
    - `random_crop=false`
    - `sliding_window_stride=512`
    - `training.num_eval_batches=null`
  - training-time best validation loss: `1.8351061700845759`
  - best step: `61500`
  - stop condition: time budget reached at `7200s`

Corrected full-coverage validation:
...
- for the current `rd` setup, `1024` full-coverage sliding windows are clearly stronger than the older `1024` random-crop recipe
- `sliding_window_stride=512` is the new official `rd` best data pipeline
