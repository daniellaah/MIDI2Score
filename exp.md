...
- keep `d_model=128`, `dim_feedforward=512`, `dropout=0.0`, `learning_rate=6e-4` as the current best 300-second candidate
- test a different class of knobs next, such as `batch_size=2` or `batch_size=8` under the same 300-second budget
- compare every new timed run against `2.5787` for the current best 300-second result
