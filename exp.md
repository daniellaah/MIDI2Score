...
- keep `d_model=128`, `dim_feedforward=512`, `dropout=0.0`, `learning_rate=6e-4`, `batch_size=8` as the new current best 300-second candidate
- use `2.2381` as the new practical timed reference for the next round of search
- the most promising next knob is likely `learning_rate` under `batch_size=8`, since batch size `8` gave the strongest improvement in this batch
